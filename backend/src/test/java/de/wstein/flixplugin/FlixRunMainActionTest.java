 ...
 *
 * <p>
 * The Flix language server sends a "Run" CodeLens for <em>every</em> entry
 * point in a file, each
 * carrying its own symbol, so dropping the argument makes the lens above one
 * entry point run a
 * different one -- silently, and only visibly wrong once a project has more
 * than one.
 *
 * <p>
 * The value arrives as JSON from a server this plugin does not control, so
 * these are as much
 * about malformed input as the happy path: anything unusable has to degrade to
 * "run the project
 * default" rather than emit {@code --entrypoint} with nothing after it, which
...
public class FlixRunMainActionTest {
    ...
    public void readsAQualifiedEntryPointSymbol() {
        // Exactly the shape Symbol.DefnSym.toString produces, and exactly what
        // --entrypoint's
    ...
    public void readsANestedNamespace() {
        // mkDefnSym's split takes everything before the *last* dot as the namespace, so
        // a nested
    ...
    public void fallsBackWhenTheServerSendsNoArguments() {
        // The case that made indexing unsafe: LSPCommand.getArgumentAt bounds-checks
        // with
        // `index > size`, so index 0 against an empty list reaches get(0) and throws,
        // turning
    ...
    public void fallsBackWhenTheArgumentIsNotAString() {
        // The wire format is JSON; a protocol change could deliver a number, a list or
        // an object.
