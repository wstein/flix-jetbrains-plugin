...
| 34 | *(added 2026-08-21)* **A function value and a lazy one read as what they are** | ✅ live in `FlixDebugSessionTest`. `{Clo$curriedMultiply$Xb8Kaq73gD3@3454}` over `clo0`, `pc`, `arg0` now reads `fn curriedMultiply(6)`: a lambda is lifted into a class named after the definition it came out of, its captures are the value, and `pc`/`arg0` are machinery. A capture-less lambda is still given one capture, of unit, so that every closure has the same shape — it is dropped, and that is now *visible* because unit renders as `()` rather than as the empty string it used to produce. A lazy value reads `lazy 4` or `lazy <unforced>`, decided by whether the unevaluated expression has been cleared — **read, never forced**: forcing it would run the program's own code to answer a question the reader asked by looking at a variable. Fault-injected three ways; the first attempt at the unit filter was **unobservable** through the label and is now covered. ⬜ **Not recorded:** capture *names* — `clo0` is a position, and the `LocalVariableTable` that maps it to `x` is keyed on JVM slots, so `fn curriedMultiply(x = 6)` needs the compiler to record them as it records a tag and a struct's fields |
