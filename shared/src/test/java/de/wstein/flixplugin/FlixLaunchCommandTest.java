 ...
 *
 * <p>
 * Every rule asserted here was learned from a failure rather than from
 * documentation, which is
 * why they are pinned: the compiler's argument parser is order-sensitive in a
 * way that turns a
 * misplaced flag into a confusing error about something else entirely, and
 * {@code --Xdebug}
...
public class FlixLaunchCommandTest {
    ...
    /**
     * What every launch used before a project could pin one; see
    ...
    public void theSubcommandComesBeforeAnyOption() {
        // The rule that matters most. An option seen before the subcommand is parsed as
        // global and
        // stops command parsing, so `flix --Xdebug build` reports `Unrecognized file
        // extension:
    ...
    public void theBuildPhasePassesXdebugExactlyOnce() {
        // A second --Xdebug after the subcommand is rejected as `Unknown option
        // --Xdebug`, so a
        // wrapper that injects one and a caller that also passes one combine into a
        // failure.
    ...
    public void theJdwpAgentGoesBeforeTheClasspathNotAfterIt() {
        // Everything from the main class on is the program's own argument list. Only
        // the JVM sees
    ...
    public void theBuildPhaseCarriesTheEntryPointAndNoAgent() {
        // --entrypoint decides which definition the generated `Main` calls, which the
        // compiler
    ...
    public void theProgramCommandNamesNoCompilerAndKeepsBothOptionListsApart() {
        // The failure the two-phase launch exists to remove: `-jar <compiler>` means
        // the JVM under
    ...
    public void bothAgentSpellingsAgree() {
        // The command-line form and the JAVA_TOOL_OPTIONS form must describe the same
        // agent, or a
    ...
    public void omitsTheEntryPointFlagEntirelyWhenAbsentOrBlank() {
        // An empty --entrypoint is rejected by the compiler, so a blank symbol has to
        // drop the flag
    ...
    public void jdwpAgentAppendsToExistingJavaToolOptions() {
        // Replacing the variable would silently drop heap or encoding settings for the
        // duration of
        // a debug session -- a difference that only appears while debugging, which is
        // the hardest
        // kind to notice.
    ...
    public void refusesToAddASecondAgentAlongsideAgentlib() {
        // Appending would give the debuggee two JDWP servers competing for suspension,
        // breakpoints
        // and lifecycle -- the state ADR 0002 forbids. The JVM's own failure is a
        // transport error
    ...
    public void refusesToAddASecondAgentAlongsideTheLegacyXrunjdwp() {
        // The pre-JVMTI spelling. Still accepted by every HotSpot, and still what older
        // tooling and
        // copied shell snippets emit, so detecting only -agentlib would miss real
        // cases.
        assertThrows(
                FlixLaunchCommand.JdwpAlreadyConfiguredException.class,
                () -> FlixLaunchCommand.withJdwpAgent(
    ...
    public void preservesOrdinaryOptionsThatMerelyMentionTheAgent() {
        // A substring match would reject this. Tokenizing does not: the value of a -D
        // property is
        // not an option that loads an agent, and refusing it would block a legitimate
        // launch.
    ...
    public void requireNoInheritedJdwpAgentRefusesAnAgentAndPassesAnythingElse() {
        // The launch path calls this before allocating a port. It is not covered by
        // withJdwpAgent's
        // own check: the run configuration puts the agent on the command line rather
        // than in the
        // environment, so nothing else would look at JAVA_TOOL_OPTIONS at all -- and
        // the debuggee
        ...
        // Must not refuse an ordinary environment, or every debug launch fails on a
        // machine that
    ...
    public void findsAPortThatCanActuallyBeBound() throws IOException {
        // The port has to be free at the moment it is handed out, which means the
        // allocator must
        // release it. A probe that stayed connected would consume the single connection
        // a JDWP
