 ...
 *
 * <p>
 * Lives in {@code shared}, beside {@link FlixLaunchCommand}, for the same
 * reason: the invocation
 * is one fact, and the modules that need it cannot depend on each other. The
 * run configuration and
 * the Tools menu both enumerate this rather than listing subcommands of their
 * own, so adding one
 * here adds it everywhere — and a name that does not exist cannot be added in
 * only one of them.
 *
 * <p>
 * Deliberately a subset. {@code lsp} and {@code lsp-vscode} are servers the
 * plugin starts
 * itself; {@code repl} needs a terminal this does not give it; {@code format}
 * would fight the
 * editor's own formatter, which the language server already provides;
 * {@code release},
 * {@code eff-lock} and the {@code X}-prefixed commands are not project
 * workflow. Every name here is
...
public enum FlixTask {
     ...
     *
     * <p>
     * Not the same thing as the Flix run/debug configuration, which exists to
     * attach a debugger
     * and therefore needs the Java plugin. This is the plain CLI run, available
     * wherever the plugin
     * loads.
     */
    ...
    /**
     * One line saying what running it does, for a tooltip or a configuration's
     * description.
     */
     ...
     *
     * <p>
     * Empty rather than a default: the value comes back from a persisted run
     * configuration, and a
     * task that was renamed or removed must not silently become a different one —
     * which, for
     * {@code clean} or {@code build-jar}, would run something destructive that
     * nobody asked for.
     */
