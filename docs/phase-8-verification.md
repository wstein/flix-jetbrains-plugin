...
| 28 | *(added 2026-08-21)* **A missing compiler is a message, not a stack trace** | ✅ `FlixClientFeaturesTest`. Without a compiler there is nothing to start, and the server said so by throwing from its constructor — with a message naming the file to fix. The message was right; where it arrived was not. LSP4IJ wraps a failed provider in `CannotStartServerException`, and the request that triggers a start is a highlighting pass, so the reason reached the user as a stack trace out of the syntax highlighter, **once per pass**, for as long as the compiler was missing. `FlixClientFeatures.isEnabled` answers before the start: no server for that project, and one balloon. The check is deliberately **not cached** — a rebuild removes the jar for a few seconds and puts it back, and a cached refusal would outlast its cause and leave the project silently unanalysed; what is remembered is the *notification*, so a missing compiler produces one balloon and not one per file. Fault-injected two ways. Seen in the wild while a compiler was being rebuilt mid-session; the `~` in that report is the IDE's own log abbreviation for `$HOME`, not an unexpanded path |
| 29 | *(added 2026-08-21)* **A tuple reads as a tuple** | ✅ `FlixValueTreeTest`, and live in `FlixDebugSessionTest` against a Datalog query result. `{Tuple$Int32$Int32@3405}` named the shape and not the value; the language writes `(exp1, …, expn)` and so does the renderer — `(1, 2)`, expanding by position, since a tuple has no field names in the source either. A tuple is the one Flix value whose compiled form is unambiguous on its own: `BackendObjType.Tuple` gives each arity and component typing a class of its own with fields `field0`, `field1`, so this needs no recorded tag and works without `--Xdebug`. Components are ordered by their index rather than by the class file's field order — fault-injected, along with the label itself |
| 30 | *(added 2026-08-21)* **A map and a set read as they are written** | ✅ `FlixValuesTest` for the format, and live in `FlixDebugSessionTest` for the walk. `Map(Node)` expanded into a red-black tree: one node per entry, named after the field holding the subtree, with the entries scattered across it. It now reads `Map#{1 => "one", 2 => "two", 3 => "three"}` and `Set#{10, 20, 30}`, expanding to entries named by key. The walk is **measured, not specified**: `RedBlackTree.Node` is `(colour, left, key, value, right)`, which is the standard library's own shape rather than the compiler's calling convention — so the live test is the real check and a stub would only repeat the measurement back. Fault-injected by swapping left/right and key/value; each fails it. Iterative with a node budget and a cycle guard, because it runs on the debugger thread against whatever the debuggee holds. Recognised by the recorded case name, so **without `--Xdebug` a map renders as the tag it is**, exactly as before |
| 31 | *(added 2026-08-21)* **A Datalog program reads as a program** | ✅ live in `FlixDebugSessionTest`. `Datalog([], [])` with `Constraint(HeadAtom, [])` under it — the AST's shape, with the program nowhere in it. It now reads `#{ Edge(1, 2). Path(x, y) :- Edge(x, y). Path(x, z) :- Path(x, y), Edge(y, z). }` and expands to one node per constraint, because everything needed survives into the value: predicate names, variable names and literals are all plain data in `Fixpoint3.Ast.Datalog`. Negation (`not`), fixity (`fix`) and the lattice separator (`P(k; v)`) are carried through, since a rule that loses one of them reads as a different rule. Two measurements corrected guesses: a body atom is `(PredSym, **Denotation**, Polarity, Fixity, terms)` where a head atom has three fields, and every literal sits in a `Fixpoint3.Boxed` wrapper whose enum is matched at the *end* of the case name — the path carries a version. Fault-injected both ways. ⚠️ **Depends on the standard library's own AST**, not the compiler's calling convention; every step checks the case it expects and gives up, so a reorganised AST degrades to the tag rendering rather than to a wrong program |
| 32 | *(added 2026-08-21)* **A solved model reads as its facts** | ✅ live in `FlixDebugSessionTest`, twice. `Model(Map)` over a red-black tree of B+ trees now reads `Model#{ Edge(1, 2). … Path(1, 4). }` — the derived closure, not just the inputs — and expands to one node per relation. It needed the compiler to record **struct field names** (row 33): the tree's nodes are structs, and until then walking it would have meant deciding from types alone which array held keys, which is a structural guess. It is read by name: `keys`, `values`, `size`, `isLeaf`, `next`. **The first version of this test was too weak and three mutations survived it** — a three-fact model fits in one B+ tree leaf, so the leaf chain was never followed and reading past `size` picked up unused slots that `contains` assertions ignored. It now asserts the model *whole*, and a second fixture (a chain of 40, closure 780) spans several leaves. Two of the three mutations fail it; the third — reading `keys` positionally — is caught by a stub instead, since in this struct `keys` happens to sit at that position |
| 33 | *(added 2026-08-21)* **A struct says what its fields are called** | ✅ fork `TestStructName`, plugin `FlixValueTreeTest`, and live. A struct is compiled to a class shared by every struct of the same erased shape, with fields named `field0`, `field1` by position — so a channel read as `MpmcAdmin(1, ReentrantLock, false, 1, …)` and the library's B+ tree could not be walked at all. The name cannot live on the class for the same reason the tag name could not, so `--Xdebug` writes it into each value: `Counter{count,label}`, the names in `field` order. A struct now reads `Counter { count = 3, label = "hits" }`. A record whose length disagrees with the field count names **nothing** rather than half of it, and without `--Xdebug` the positional names remain — both fault-injected |
| 34 | *(added 2026-08-21)* **A function value and a lazy one read as what they are** | ✅ live in `FlixDebugSessionTest`, plus `FlixValueTreeTest` for the naming rules. `{Clo$curriedMultiply$Xb8Kaq73gD3@3454}` over `clo0`, `pc`, `arg0` now reads `fn curriedMultiply(x = 6)`: a lambda is lifted into a class named after the definition it came out of, its captures are the value, and `pc`/`arg0` are machinery. The capture *names* come from a `cloNames` constant a `--Xdebug` build writes **on the class** — a closure class belongs to one lambda, so unlike a tag or a struct name the names are the same for every instance (fork `TestCaptureNames`). A capture the source never named is recorded as `_` so the list still lines up with `clo0`, and reads by position; names that do not line up with the captures are **not used at all**, as with a struct. A capture-less lambda is still given one capture, of unit, and it is dropped — visible now only because unit renders as `()` rather than as the empty string it used to produce. A lazy value reads `lazy 4` or `lazy <unforced>`, decided by whether the unevaluated expression has been cleared — **read, never forced**. Fault-injected six ways; the first attempt at the unit filter was **unobservable** through the label and is now covered |
| 35 | *(added 2026-08-21)* **An anonymous object names what it implements** | ✅ live in `FlixDebugSessionTest`, with the type rules in `FlixValueTreeTest`. `{Anon$220078@3745}` is an id and nothing else; what identifies the value is the Java type it implements and the methods it defines, so it reads `new Comparator { compare }` and expands to `compare = fn main(bias = 7)` — the closure that implements it, rendering as itself. Methods pair with `clo0`, `clo1` by position, which is the order `GenAnonymousClasses` writes both in; constructors, bridges and synthetics are skipped, since a bridge exists so a closure can call back into the object and implements nothing the source wrote. An interface is implemented and a class is extended, so the type is the first interface or else the superclass — and **`java.lang.Object` is not an answer**, being what a class implementing an interface extends; with nothing else to go on the generated name is honest. Fault-injected six ways: three through the live session, and three through stubs for the cases live data cannot produce (no interface, a real superclass, and `Object`) |
| 36 | *(added 2026-08-21)* **A breakpoint watches named classes, not every class** | ✅ fork `TestDebugIndex`, `FlixDebugIndexTest`, and live in `FlixDebugSessionTest`. A generated name encodes the symbol and never the file (`Clo$main$626ZYxrpg1N`), so the only correct request was one watching **every** class the VM prepares — one event per class loaded, per breakpoint. A `--Xdebug` build now writes `build/development/debug-index.json`, source → classes, and the position manager creates one exact-name request per class instead. The index is **read back from the emitted `SourceFile`/`SourceDebugExtension` attributes** of the classes that were written, not collected from the generators: a lifted class takes its primary source from its own location and inlining puts foreign lines in through SMAP, and it is the emitted attributes a debugger reads at runtime. Many-to-many in both directions, so it is **not** an account of which source *owns* a class. `FlixLineOnly` is unchanged — the index narrows the watch, and whether a class can host the *line* is still asked per class. Absent index (no build, no `--Xdebug`, older compiler) falls back to watching everything. Fault-injected six ways, three of them live: an index that names classes subtly wrong, one ignored entirely, and a compiler that omits a class from it — each caught by `a breakpoint watching only the classes the debug index names still binds and hits`, which arms one exact-name request per named class against a real VM. Measured, in `FlixDebugSessionTest`: the named watch received **1** class-prepare event, against **457** classes the VM had loaded by the time the breakpoint hit — which is what an unfiltered request has to receive and re-check, once per class, for every breakpoint in the session. The margin is asserted as an order of magnitude rather than as a figure: how many JDK classes a Flix program loads is the JDK's business and moves between releases, and what is being pinned is that this is a different kind of watch rather than a slightly smaller one |
| 37 | *(added 2026-08-21)* **The Flix build no longer runs on the EDT** | ✅ `FlixDebugPhaseOrderTest`, fault-injected four ways. Phase one ran inside `startProcess`, under a comment asserting that the platform calls it off the EDT. It does not: `ExecutionManagerImpl.doRun` → `runInEdt` → `GenericDebuggerRunner.doExecute` → `createContentDescriptor` → `DefaultDebugEnvironment.createExecutionResult` → `CommandLineState.execute` → `startProcess`, and `doExecute` opens with `saveAllDocuments()`, which requires the EDT. A cold Flix build therefore froze the IDE for minutes, reported as `Synchronous execution on EDT` from `OSProcessHandler.checkEdtAndReadAction`. `FlixDebuggerRunner` now runs phase one in a cancellable `Task.Backgroundable` and re-enters the platform's runner from `onSuccess`. A `BeforeRunTask` was the other candidate and was rejected: it is user-removable from *Before launch*, and removing it would debug whatever the last build left. ⬜ **Not measured:** the gesture — that pressing Debug shows a progress bar rather than a frozen IDE — which needs the UI harness and a compiler |
| 38 | *(added 2026-08-21)* **A paused frame's names carry their Flix type** | ✅ fork `TestDebugScopes` (8 tests, fault-injected five ways) and `FlixDebugScopesTest` (8 tests, fault-injected three ways). Step 1 of the evaluation programme. A `--Xdebug` build writes `build/development/debug-scopes.json`: *(class, method, name) → Flix type*, for every parameter and capture. The `LocalVariableTable` already gives a name, a slot and a range; the type is what it cannot give, since the back end erases. Measured, not assumed — an enum parameter's descriptor is the shared `Tagged$`, which says neither which enum nor its arguments, while a **tuple** turns out to be self-describing (`Tuple$Int32$Bool`) and gains nothing, recorded rather than hidden. Slots are **not** repeated: two tables naming slots would be two chances to disagree. Derived from the reduced AST rather than collected during code generation, which runs in parallel; the one rule it must not restate — which class a definition becomes — is shared with the emitter as `GenFunAndClosureClasses.classNameOf`. ⬜ **Not covered, deliberately:** `let`-bound locals, whose slots are allocated inside `GenExpression`; and a function parameter's *result* type, erased before this table exists (`Int32 -> String` is recorded as `(Int32) -> java.lang.Object`). ⬜ **Not measured:** anything reading it — nothing consumes the table yet, which is the next step |
| 39 | *(added 2026-08-21)* **An expression can be typed against the frame it is paused in** | ✅ fork `TestDebugEvalProvider` (10 tests) and `TestDebugEvalWire` (7). Step 2 of the evaluation programme, and the first thing to *read* the scope table. `flix/debugEval/compile` takes a frame — a generated class and a method, which is what JDI reports and what no document position identifies — plus an expression and a policy, and answers with the expression's type and effect. It runs nothing. The table's own recorded types **cannot** do this: monomorphisation has already replaced `Option[String]` with `Option$AxNJjn6TiM2` and dropped the arguments, measured. What the table supplies is the **defining symbol**, added for this; the types come from the server's typed AST under that symbol, so the answer is the compiler's rather than the IDE's guess at it. Typed by compiling a wrapper whose parameters are the scope and whose body binds the expression — a `let` because the return type is what is being asked. Three shapes of answer, kept apart: `ok`, `failed` (the compiler's diagnostics about the expression), `rejected` (this server declining, naming what must change). A rejection is not a JSON-RPC error: "you have not built with --Xdebug" must not arrive in the same channel as "the server crashed". Fault-injected four ways. The IntelliJ side is now wired: `FlixDebugEval` in `language` is the shape of the question, `FlixDebugEvalClient` in `backend` speaks LSP, and `debugger` asks by interface — which is the only arrangement the module boundary allows, since the frame comes from one and the answer through the other and neither may depend on the other. `FlixLanguageServerFactory.getServerInterface` names a wider interface so a non-standard request has somewhere to go. Fault-injected three ways. `FlixCodeFragmentFactory` now calls it, on the one path where it helps: an expression the frame *can* read is read, since a watch re-evaluates on every step and typing `at` again on each would spend a compilation to confirm what is already in hand. An expression it cannot read is put to the compiler, and the three answers are kept apart — the compiler's own diagnostics when the expression is wrong; the type and effect plus "running it is not implemented" when it is right; and the local limit when the compiler cannot say. The middle case is the point: `List.length(xs)` used to be answered with a sentence about record projections, which reads as though the expression were at fault. Fault-injected four ways. ⬜ **Not measured:** a request crossing to a running server — every test stubs the service, and the live half needs an IDE, a started server and a paused debuggee at once. ⬜ **Not built, deliberately:** the artifact, `DebugEvalHost`, and any execution; and the recompile is whole-project per request, which the cached sidecar (step 5) exists to fix |
