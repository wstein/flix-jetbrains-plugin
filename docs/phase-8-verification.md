...
| 32 | *(added 2026-08-21)* **A solved model names its relations** | ⚠️ **relations, not facts** — and the boundary is the point. `Model(Map)` over a red-black tree of B+ trees now reads `Model#{ Edge/2, Path/2 }`, expanding to one node per relation: the *symbol* is exact (`RelSym.Symbol` carries a predicate name, an arity and a denotation, so a lattice relation says so). The **facts are not read**: they live in a `BPlusTree` whose nodes are `Struct$…` classes shared by every struct of the same erased shape, so the field names are nowhere in the value and walking the tree would mean deciding from arity and types alone which array holds keys and which holds children — a structural guess, which is the one thing this plugin does not do with a value. A reader who wants the facts has them exactly: `query … select … from P(…)` returns a vector of tuples, which renders as tuples. Live, fault-injected two ways. **What would unlock the facts**: recording struct field names under `--Xdebug`, the same shape of change as the tag name — it would also make every user struct (a channel, for instance) legible, since those show as `field0…fieldN` today |
