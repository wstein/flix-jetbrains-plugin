...
/**
 * Whether a project can have a language server, and what is said when it cannot.
 *
 * <p>The answer used to be given by starting the server and letting it fail: LSP4IJ wraps a failed
 * provider in {@code CannotStartServerException}, and the request that triggers a start is a
 * highlighting pass -- so the reason reached the user as a stack trace out of the syntax
 * highlighter, once per pass, for as long as the compiler was missing. Asking first turns that into
 * a project with no server and one balloon.
 *
 * <p>A real project on disk ({@link HeavyPlatformTestCase}) for the reason {@code FlixForkTest}
...
public class FlixClientFeaturesTest extends HeavyPlatformTestCase {
    ...
    public void testTheProblemSaysWhatToDoAboutIt() {
        // The state a new project is in. A refusal that does not name the file to supply is a
    ...
    public void testTheAnswerFollowsTheCompilerRatherThanBeingCached() throws IOException {
        // A cached refusal would outlast its cause: a rebuild removes the jar for a few seconds,
    ...
    public void testTheSameProblemIsReportedOnce() {
        // One balloon per problem, not one per file: `isEnabled` is asked for every file the editor
