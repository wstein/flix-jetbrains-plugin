...
- `--Xdebug` is not only a JDWP switch: `Let`, `ApplyDef`, `ApplyClo`, `IfThenElse` and `Stm` emit
  ...
  names frame slots, records where each `pc` resumes (`pcLines`), and writes the **case name** into
  a tagged value (`Tagged$.tag`) — a case with terms is compiled to a class shared by every case of
